namespace Template {
  export async function Szene1_5(): ƒS.SceneReturn {
    ...
    let text = {
      MainNarrator: {
        ...
        T1007: ". . . erleuchten?",
        T0006:
          "Es ist besser ein Licht anzuzünden, als die gesamte Dunkelheit zu verfluchen, findest du nicht?",
        T0007:
          "Da wir uns gerade besser kennenlernen, möchtest du deinen Namen mit mir teilen?",

        T0008: "Das ist sehr lieb von dir!",
        T0009: ". . .",
        T0010:
          "Spann mich nicht so auf die Folter. Sag schon, wie ist dein Name?",
        ...
        T0016:
          "An deiner Stelle hätte ich mir sowieso einen besseren Namen ausgedacht",
        ...
        T0021:
          "Dich erwartet nun eine heldenhafte Reise in ein längst vergessenes Königreich voller Herausforderungen, Tücken und Gefahren.",
        T0022:
          "Bevor ich dir diese Geschichte also zumute, muss ich wissen. . .",
        ...
        //Lichtaus
        L0004:
          "Willst du wirklich weiter so vor einem dunklen Bildschirm sitzen?",
        ...
        G0006:
          "Wenn du rausgefunden hast, wie man von diesem Ort entkommen kann. . .",
        G0007: "Sei so gut und lass es mich bitte wissen, ja?",
        //Nach der Umgebung fragen
        G0008: "Wo wir sind?",
        G0009: "Ich bin mir nicht ganz sicher",
        G0010:
          "Alles scheint vertraut zu sein, verhält sich jedoch sehr seltsam",
        G0011:
          "Je länger du dich hier aufhältst, desto mehr Erinnerungen verschwinden aus deinem Gedächtnis",
        G0012: ". . .",
        ...
        T0036:
          "Die Haftung meinerseits, für körperliche oder geistige Schäden wird hiermit ausgeschlossen. . . heh",
    ...
    await ƒS.Character.show(
      characters.narrator,
      characters.narrator.pose.eyesonly,
      ƒS.positionPercent(40, 80)
    );
    await ƒS.update(3);
    ...
    await ƒS.Sound.play(sound.buttonhover, 0.2, false);
    let lichtanschalten = {
      lichtan: "Licht anschalten",
      lichtaus: "Nichts tun",
    };
    let lichtanschaltenelement = await ƒS.Menu.getInput(
      lichtanschalten,
      "auswahl"
    );
    switch (lichtanschaltenelement) {
      case lichtanschalten.lichtan:
        ...
        ƒS.Character.show(
          characters.narrator,
          characters.narrator.pose.standard,
          ƒS.positionPercent(10, 80)
        );
    ...
    switch (
      erstesdialogelement //Wenn Auswahl "Namen eingben"
    ) {
      case entscheidung1.iSayYes:
        await buttonpress(sound.buttonpress, 0.2);
        await satzbau(
          characters.MainNarrator,
          text.MainNarrator.T0008,
          true,
          true,
          4,
          50,
        ...
        await satzbau(
          characters.MainNarrator,
          dataForSave.protagonist.name + " also?",
          true,
          true,
          4,
          50,
    ...
    await satzbau(
      characters.MainNarrator,
      dataForSave.protagonist.name + text.MainNarrator.TT001,
      true,
      true,
      5,
      50,
    ...
    while (i < 3) {
      ...
      //Abhängig von der oben stehenden Entscheidung wird nun folgendes passieren:
      switch (
        tutorialauswahl //Wenn Auswahl "Namen eingben"
      ) {
        case tutorial[0]:
          await buttonpress(sound.buttonpress, 0.2);
          await satzbau(
            characters.MainNarrator,
            text.MainNarrator.TT010,
            true,
            true,
            4,
            50,
    ...
    await satzbau(
      characters.MainNarrator,
      text.MainNarrator.T0029 + " " + dataForSave.protagonist.name + "?",
      false,
      false,
      4,
      50,
    ...
    while (mam(x) < 3) {
      ...
      switch (
        geschichtenauswahl //Wenn Auswahl "Namen eingben"
      ) {
        case geschichten[0]:
          await buttonpress(sound.buttonpress, 0.2);
    ...
    let herausforderungselement = await ƒS.Menu.getInput(
      herausforderung,
      "auswahl"
    );
    ...
    let türöffnen = {
      iSayYes: "Tür normal öffnen", //Passiert a
      iSayNo: "Tür langsam öffnen", //Passiert b
    };
    let türöffnenelement = await ƒS.Menu.getInput(türöffnen, "auswahl");
    switch (türöffnenelement) {
      case türöffnen.iSayYes:
        await buttonpress(sound.buttonpress, 0.2);
        ƒS.Sound.play(sound.dooropen, 1, false);
        break;
